...
router.get('/session', authenticateJWT, async (req, res) => {
  ...
  let isAborted = false;
  for (let i = 0; i < 10; i++) {
    TestModel({ username: '111' + i, password: 'ppp', name: '333' }).save(
      { session: session },
      async (err, data) => {
        if (err) {
          if (isAborted) return;
...
router.get('/add_random_product_data', authenticateJWT, async (req, res) => {
  ...
  for (let i = 0; i < arrColorRef.length; i++) {
    ...
    await ColorModel.updateMany(
      { _id: colorRef },
      { $inc: { relatedProductCount: ++objColorRefIncCount[colorRef] } }
    );
  }
...
function generateRandomProductCode() {
  ...
  // AA0001
  return (
    arrAlphabet[generateRandomIntegerNumber(0, arrAlphabet.length - 1)] +
    arrAlphabet[generateRandomIntegerNumber(0, arrAlphabet.length - 1)] +
    generateRandomIntegerNumber(0, 9) +
    generateRandomIntegerNumber(0, 9) +
    generateRandomIntegerNumber(0, 9) +
    generateRandomIntegerNumber(0, 9)
  );
...
function generateRandomColorRef() {
  const arrColorId = [
    '6018bc26884f1e2ec5b54490',
    '6018bc29884f1e2ec5b54491',
    '6018bc2b884f1e2ec5b54492',
    '6018bc2d884f1e2ec5b54493',
    '6018bc31884f1e2ec5b54494',
    '601a8e9aa76e1952c1a1786f',
    '601a8e9fa76e1952c1a17870',
    '601a8ea4a76e1952c1a17871',
    '601a8eb0a76e1952c1a17872',
    '601a8eb3a76e1952c1a17873',
    '601a8ebca76e1952c1a17874',
    '601a8ec5a76e1952c1a17875',
    '601a8ecaa76e1952c1a17876',
    '601a8ecda76e1952c1a17877',
    '601a8ed0a76e1952c1a17878',
    '601a8edca76e1952c1a17879',
    '601a8ee7a76e1952c1a1787a',
    '601a8eeaa76e1952c1a1787b',
    '601a8eeea76e1952c1a1787c',
  ];
...
function generateRandomCompanyName() {
  ...
  for (let i = 0; i < intCompanyNameLength; i++) {
    strCompanyName +=
      arrCompanyNameWord[
        generateRandomIntegerNumber(0, arrCompanyNameWord.length - 1)
      ];
  }
  let strCompanyIndustry =
    arrCompanyIndustry[
      generateRandomIntegerNumber(0, arrCompanyIndustry.length - 1)
    ];
...
function generateRandomName() {
  ...
  if (nameLength === 2) {
    return (
      firstWord[generateRandomIntegerNumber(0, firstMax - 1)] +
      secondWord[generateRandomIntegerNumber(0, secondMax - 1)]
    );
  } else {
    return (
      firstWord[generateRandomIntegerNumber(0, firstMax - 1)] +
...
function generateRandomPhoneNumber() {
  ...
  const strPhonePrefix =
    arrPhonePrefix[generateRandomIntegerNumber(0, arrPhonePrefix.length - 1)];
...
function generateRandomPlateNumber() {
  ...
  // 10000-99999
  return (
    arrPlatePrefix[generateRandomIntegerNumber(0, arrPlatePrefix.length - 1)] +
    String(generateRandomIntegerNumber(10000, 99999))
  );
}
